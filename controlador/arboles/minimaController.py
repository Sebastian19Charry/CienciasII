# controlador/arboles/minimaController.py
...
class MinimaController:
    """
    Controlador para el Árbol de Expansión Mínima.
    Algoritmo: Kruskal con UnionFind (union por rango + path compression).
    
    Genera:
    - Info general: ramas, cuerdas, peso total
    ...
    # ── Algoritmo principal: Kruskal Mínimo ──────────────────────────────────
    ...
    def _mostrar_resultados(self):
        ...
        # Tab Info
        html = f"<h3>Árbol de Expansión Mínima</h3>"
        ...
        self._dlg("Árbol Generado",
                  f"Árbol de expansión mínima generado.\nPeso total: {self.peso_total}")
    ...
    def _analizar_conjuntos_corte(self):
        ...
        for tamano in range(1, max_t + 1):
            for combo in combinations(self.aristas, tamano):
                ...
                # Saltar si contiene un conjunto ya encontrado
    ...
    def _generar_matrices(self):
        ...
        # Matrices de circuitos fundamentales
        if cuerdas:
            todas = list(self.aristas)

            # Matriz CF vs Aristas (0/1)
...
# ── Helpers globales (reutilizados por ambos controladores) ──────────────────
class _UnionFind:
    """Union-Find con path compression y union por rango."""
    def __init__(self, n):
...
def _bfs_camino(u, v, aristas_arbol, n_vertices):
    """BFS para encontrar el camino entre u y v en el árbol."""
    grafo = {i: [] for i in range(n_vertices)}
...
def _es_conexo(aristas, n_vertices):
    """Verifica si el grafo formado por las aristas dadas es conexo."""
    if not aristas:
        return False
    grafo = {i: [] for i in range(n_vertices)}
    for a, b in aristas:
    ...
    while cola:
        actual = cola.popleft()
        for v in grafo[actual]:
            if v not in visitados:
                visitados.add(v)
                cola.append(v)
    return len(visitados) == n_vertices
