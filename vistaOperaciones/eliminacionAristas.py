...
class InterfazEliminacionAristas(QWidget):
    def __init__(self, main_window):
        ...
        self.btn_resolver = QPushButton("⚡ ELIMINAR ARISTA")
        self.btn_resolver.setFixedHeight(32); self.btn_resolver.clicked.connect(self.resolver_eliminacion_arista)
        del_lay.addWidget(self.input_arista); del_lay.addWidget(self.btn_resolver)
        ...
        # TÍTULO
        self.main_title = QLabel("OPERACIÓN: ELIMINACIÓN DE ARISTAS")
        ...
        self.g_res = EditorGrafoContenedor("RESULTADO (Arista eliminada)", modo_lectura=True)
    ...
    def resolver_eliminacion_arista(self):
        target_nombre = self.input_arista.text().strip()

        # Validación
        existe_arista = any(a.nombre == target_nombre for a in self.g1.aristas)

        if not existe_arista:
            QMessageBox.warning(self, "Error", f"La arista '{target_nombre}' no existe en el grafo.")
            return
        ...
        # 1. Copiar TODOS los vértices (En esta operación NO se borran vértices)
        ...
        # 2. Copiar solo las aristas que NO coinciden con el nombre ingresado
        for a in self.g1.aristas:
            if a.nombre != target_nombre:
