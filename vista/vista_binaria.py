...
class VistaBusquedaBinaria(QWidget):
    def __init__(self, main_window):
        ...
        # === TÍTULO ===
        lbl_titulo = QLabel("Búsqueda Binaria")
        ...
        top_panel.setStyleSheet(
            "background: white; border-radius: 15px; border: 1px solid #d8e3dc;"
        )
        ...
        # === BOTONES ===
        self.btn_crear = self.boton("➕ Crear")
        self.btn_insertar = self.boton("📥 Insertar")
        self.btn_buscar = self.boton("🔍 Buscar")
        self.btn_eliminar = self.boton("🗑️ Eliminar")
        self.btn_deshacer = self.boton("↶ Deshacer")
        self.btn_reset = self.boton("♻️ Limpiar")
        self.btn_man_save = self.boton("💾 Guardar")
        self.btn_man_load = self.boton("📂 Recuperar")
        ...
        split.addWidget(self.tabla, 2)
        split.addWidget(self.salida, 3)
    ...
    def boton(self, texto):
        ...
        btn.setStyleSheet(
            "background:#2d6a4f; color:white; font-weight:bold; border-radius:8px; padding: 4px 12px; font-size: 11px;"
        )
        return btn
    ...
    def actualizar_tabla(self, datos):
        ...
        for i in range(self.tabla.rowCount()):
            ...
            if i < len(datos):
                self.tabla.setItem(i, 1, QTableWidgetItem(str(datos[i])))
            else:
                self.tabla.setItem(i, 1, QTableWidgetItem(""))
