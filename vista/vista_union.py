...
class VistaUnion(QWidget):
    def __init__(self, main_window, parent_nav):
        ...
        titulo = QLabel("Unión de Grafos  (G₁ ∪ G₂)")
        ...
        # — Acción central —
        self.btn_calc = self.boton_accion("∪ UNIR")
        self.btn_calc.setStyleSheet(
            self.btn_calc.styleSheet() +
            "QPushButton { background:#1b4332; min-width:110px; }")
    ...
    def agregar_a1(self):
        ...
        if dlg.exec():
            ...
            aristas_norm = [tuple(sorted(x)) for x in self.grafo1_aristas]
            if a not in aristas_norm:
                ...
                DialogoClave(0, "Advertencia", "mensaje", self,
                             "Esa arista ya existe en G₁.").exec()
    ...
    def agregar_a2(self):
        ...
        if dlg.exec():
            a = tuple(sorted(dlg.get_arista()))
            aristas_norm = [tuple(sorted(x)) for x in self.grafo2_aristas]
            if a not in aristas_norm:
    ...
    def eliminar_a1(self):
        ...
        if dlg.exec():
            a = tuple(sorted(dlg.get_arista()))
            aristas_norm = [tuple(sorted(x)) for x in self.grafo1_aristas]
            if a in aristas_norm:
                self.grafo1_aristas.pop(aristas_norm.index(a))
                self.vis1.set_grafo(self.grafo1_vertices,
                                    self.grafo1_aristas, self.grafo1_etiquetas)
            else:
                DialogoClave(0, "Error", "mensaje", self,
                             "Esa arista no existe en G₁.").exec()
    ...
    def eliminar_a2(self):
        ...
        if dlg.exec():
            ...
            aristas_norm = [tuple(sorted(x)) for x in self.grafo2_aristas]
            if a in aristas_norm:
                self.grafo2_aristas.pop(aristas_norm.index(a))
                self.vis2.set_grafo(self.grafo2_vertices,
                                    self.grafo2_aristas, self.grafo2_etiquetas)
            else:
                DialogoClave(0, "Error", "mensaje", self,
    ...
    def guardar_g1(self):
        ...
        if path:
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"vertices": self.grafo1_vertices,
                           "aristas": self.grafo1_aristas,
                           "etiquetas": self.grafo1_etiquetas}, f, indent=2)
        ...
        if path:
            try:
                with open(path, "r", encoding="utf-8") as f:
                    d = json.load(f)
                self.grafo1_vertices = d["vertices"]
                self.grafo1_aristas = [tuple(a) for a in d["aristas"]]
                self.grafo1_etiquetas = {int(k): v
                                          for k, v in d.get("etiquetas", {}).items()}
    ...
    def guardar_g2(self):
        ...
        if path:
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"vertices": self.grafo2_vertices,
                           "aristas": self.grafo2_aristas,
                           "etiquetas": self.grafo2_etiquetas}, f, indent=2)
    ...
    def cargar_g2(self):
        ...
        if path:
            try:
                with open(path, "r", encoding="utf-8") as f:
                    d = json.load(f)
                self.grafo2_vertices = d["vertices"]
                self.grafo2_aristas = [tuple(a) for a in d["aristas"]]
                self.grafo2_etiquetas = {int(k): v
                                          for k, v in d.get("etiquetas", {}).items()}
    ...
    def calcular(self):
        ...
        e1 = set(self.grafo1_etiquetas.values())
        e2 = set(self.grafo2_etiquetas.values())
        union_etiq = e1.union(e2)

        etiq_res = {i: e for i, e in enumerate(sorted(union_etiq))}
        map_res = {e: i for i, e in etiq_res.items()}

        aristas_res = set()
        for a in self.grafo1_aristas:
            et_o = self.grafo1_etiquetas[a[0]]
            et_d = self.grafo1_etiquetas[a[1]]
            aristas_res.add(tuple(sorted([map_res[et_o], map_res[et_d]])))
        for a in self.grafo2_aristas:
            et_o = self.grafo2_etiquetas[a[0]]
            et_d = self.grafo2_etiquetas[a[1]]
            aristas_res.add(tuple(sorted([map_res[et_o], map_res[et_d]])))

        self.vis_res.set_grafo(len(union_etiq), list(aristas_res), etiq_res)
        DialogoClave(0, "Unión calculada", "mensaje", self,
                     f"Vértices: {len(union_etiq)}\n"
                     f"Aristas: {len(aristas_res)}").exec()
